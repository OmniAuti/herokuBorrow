...
const AccountDashboardBorrowedInterest = ({accountItemsData, isItemsLoaded}) => {
    ...
    return (
       ...
       {!isItemsLoaded ? <div className="flex flex-wrap"><AccountSupplyLoading/><AccountSupplyLoading/><AccountSupplyLoading/></div> :
        <div className="flex flex-wrap">
          {accountItemsData.filter(data => data.postType === 'borrow').length > 0 ? accountItemsData.filter(data => data.postType === 'borrow').map(data => <AccountSupplyObjectCard key={data._id} data={data}/>) : <p className="text-2xl w-full font-thin text-center">You have not shown an interest in any offered supplies</p>}
        </div>}
      </div>
    )
}

export default AccountDashboardBorrowedInterest;